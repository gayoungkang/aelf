...
<%
	...
	String dateStr[] = pi.getPiDate().split(" ");
	...
	String Contents = CryptoSlate.removeTag(pi.getPiContent());
...
<meta name="Title" content="<%=pi.getPiTitle() %>">
...
<meta property="og:title" content="<%=pi.getPiTitle() %>">
<meta property="og:description" content="<%=Contents%>">
<meta property="og:image" content="http://www.aelfkorea.io/img/og_thumbnail.jpg">
<meta property="og:url" content="http://www.aelfkorea.io/petition.lf?menu=petition&mode=view&gubun=recent&pi_no=<%=pi.getPiNo()%>">

<meta name="apple-mobile-web-app-title" content="<%=pi.getPiTitle() %>">
        ...
        <div class="board">
            <div class="board-1">
                <table cellpadding="0" cellspacing="0">
                    <tr height="56">
                        <td colspan="5" style="font-size: 16px; font-weight: 500; border-bottom:1px solid #e0e0e0; padding:0 20px;" class="notice-td-1"><%=pi.getPiTitle() %></td>
                    </tr>
                    <tr height="53">
                        ...
                        <td style="border-bottom:1px solid #e0e0e0;"><span style="padding:0 10px;">｜</span><%=nf.format(pi.getPiView()) %>명</td>
                        <td width="335" style="border-bottom:1px solid #e0e0e0;"></td>
                    </tr>
                    <tr height="190">
                        <td class="notice_content" colspan="5" style="letter-spacing: -0.8px; vertical-align: top; word-break:break-all;"><%=pi.getPiContent() %></td>
                    </tr>
					
					<%if("".equals(pi.getPiReply())==false) { %>
                    <tr>
                    	<td colspan="5" style="letter-spacing: -0.5px; vertical-align: top; padding:20px; ">
	                    	<div class="notice_content petition_content" style="word-break:break-all; ">
	                    		<p>답변 내용</p>
	                    		<div>
	                    			<span><%=pi.getPiReply().replaceAll("\n", "<br/>") %><br></span>
	                    		</div>
	                 		</div>
                 		</td>
                    </tr>
                    ...
                    <tr>
                    	<td class="notice-like-wrap" colspan="5">
                    		<img src="img/notice-like.png" onMouseOver="this.src='img/notice-like-on.png'" onMouseOut="this.src='img/notice-like.png'" onclick="javascript:likeHandler(<%=pi.getPiNo() %>, 'petition_info', <%=pi.getUiNo() %>)" alt="" />
                ...
                <table cellpadding="0" cellspacing="0" style="width: 100%;">
                    ...
                    <tr height="10">
                    	...
                    	<%if("desc".equals(sortType)) { %>
                    		<button style="width: 100px; padding: 0px;" onclick="javascript:location.href='petition.lf?menu=petition&mode=view&pi_no=<%=pi.getPiNo() %>&sort_type=asc'">날짜 순서로</button>
                    	<%} else { %>
                    		<button style="width: 100px; padding: 0px;" onclick="javascript:location.href='petition.lf?menu=petition&mode=view&pi_no=<%=pi.getPiNo() %>&sort_type=desc'">날짜 역순으로</button>
                    	<%} %>
                    	</td>
                    </tr>
                    <%} %>
							...
							<table cellpadding="0" cellspacing="0" style="width: 100%;">
								<tr>
									...
									<%if(reply.getNrDelete()==0) { %>
										<button onclick="javascript:likeHandler(<%=reply.getNrNo() %>, 'petition_reply', <%=reply.getUiNo() %>)">좋아요</button>&nbsp;&nbsp;
